...
/**
 * ★ 위와 **같은 문장**을, 앞쪽 한 줄만 `/admin/copy` 저장값으로 갈아 끼운 것.
 *   (2026-09-16 지시 [9] — 권반장 목록에 없던 **다섯 번째 곳**을 찾아 함께 배선했다)
 * ⚠ 뒤쪽 `PENDING_NOTE`(「지금 나가는 곳은 N곳…」)는 **갈아 끼우지 않는다.**
 *   그것은 문구가 아니라 `live` 값에서 저절로 나오는 **사실**이다. 손으로 적게 하면
 *   2026-09-06 의 「182곳 중 175곳이 어긋난」 사고를 문구에서 되풀이하게 된다.
 * ⚠ `snsLine` 을 안 넘기면 위 상수와 **글자 하나까지 같다**(기본값이 `CHANNELS_LINE_MARKED`).
 */
export function channelsPitch(snsLine: string): string {
  return `${snsLine}. ${PENDING_NOTE}`;
}
