...
export type UploadInput = {
  ...
  /**
   * ★ **그 SNS 에만 있는 것.** (2026-09-12 — 틱톡 때문에 생겼다)
   *
   * 틱톡은 심사 요건상 **올릴 때마다 사장님이 제목·공개범위·댓글 허용을 직접 골라야** 한다.
   * 그 고른 값이 여기 실린다(`lib/sns/tiktok.ts` 의 `TiktokChoice`).
   *
   * ⚠ **다른 어댑터는 이 칸을 보지 않는다.** 선택 항목이라 인스타·유튜브 코드는 한 글자도 안 바뀐다.
   * ⚠ 여기에 «공통 기능»을 넣지 마라. 공통이면 위에 제 이름을 가진 칸으로 올려야 한다 —
   *   여기가 잡동사니 서랍이 되면 어댑터마다 규칙이 달라진다.
   */
  extra?: unknown;
};
