 ...
 *
 * ⚠⚠ **[48]② 는 「7일 → 1일」이었는데 «안 줄였습니다». 이유가 생겼습니다.**
 *
 * 🔴 2026-09-17 실측: 이 판정(`isPostAlive`)을 한 번 돌렸더니 **살아 있어 보이는 글 2건을
 *   2건 다 「없다」**고 답했습니다. 그 주소를 직접 열어 봐도 **밖에서는 가릴 수가 없었습니다**
 *   (인스타는 비로그인에게 있는 글이든 없는 글이든 **같은 껍데기**를 줍니다).
 *   ⇒ 우리가 아는 것은 **「우리 토큰으로는 안 보인다」**까지다 — 지워진 것일 수도,
 *     **비공개·권한 문제**일 수도 있다.
 *
 * ⇒ **판정이 못 미더운 채로 «7배 자주» 돌리면 «틀린 결과»가 7배가 된다.**
 *   멀쩡한 「인스타에서 보기」가 손님 화면에서 7배 빨리 사라진다는 뜻이다.
 * ⇒ **판정이 맞다는 것이 확인된 뒤에 줄인다.** 급한 자리는 그 사이
 *   **사장님에게 알리는 쪽**(`app/api/site/videos/recheck` — 감추지 않고 알리기만)이 맡는다.
 *
 * ⚠ **호출이 7배가 되지 않는다 — 세어 봤다.**
 *   한 번에 물어보는 수는 아래 `RECHECK_LIMIT`(50건)가 이미 막고 있다.
 *   크론은 **하루 한 번** 도니 며칠짜리로 바꾸든 **하루 최대 50건**으로 같다.
 *   ⇒ 바뀌는 것은 **「같은 글을 얼마나 자주 다시 보나」**이지 **하루 총 호출 수가 아니다.**
 * ★ 그리고 급한 자리는 크론이 아니라 **사장님이 편집화면을 열 때**가 맡는다([48]① ·
 *   `app/api/site/videos/recheck`). 크론은 «그 뒤를 받치는» 그물이다.
 */
