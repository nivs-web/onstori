 ...
 *
 * ⚠⚠ **[48]② 는 「7일 → 1일」이었는데 «아직도 안 줄였습니다». 2026-09-17 밤에 한 번 더 따졌습니다.**
 *
 * ## 2026-09-17 — 권반장이 «줄여도 된다»고 하셨으나, **그 근거가 이 파일에는 안 맞습니다**
 *
 * > 권반장: 「판정이 맞다는 것이 확인됐다 ✅ / **그리고 지금 설계는 «감추지 않고 알리기만» 이다**
 * >   ⇒ 비공개를 「없다」로 잡아도 **링크가 사라지지 않습니다**」
 *
 * 🔴🔴 **「감추지 않고 알리기만」은 «다른 파일» 이야기입니다.**
 *   그건 `app/api/site/videos/recheck/route.ts`(사장님이 편집화면을 열 때) 입니다 —
 *   그 창구는 일부러 `remote_deleted_at` 을 **안 씁니다.**
 * 🔴 **이 파일(크론)은 «감춥니다».** 바로 아래 `checkPublishedAlive` 가 `gone` 판정에
 *   **`remote_deleted_at` 을 적고**, `lib/shorts.ts` 가 `remote_deleted_at IS NULL` 로 걸러
 *   **손님 화면에서 「인스타에서 보기」가 사라집니다.**
 *   ⇒ 여기서 7→1 로 줄이면 **틀린 판정이 링크를 «7배 빨리» 지웁니다.** 알림이 한 번 더 뜨는 게 아닙니다.
 *
 * ★ 권반장도 **바로 그 조건을 스스로 다셨습니다** — 「**「감추기」로 바꾸는 날에는 주기를 다시 늘릴지
 *   먼저 따지십시오. 그때는 틀린 판정이 진짜로 링크를 지웁니다**」.
 *   그 「그날」이 **미래가 아니라 «지금»**입니다. 그래서 그 조건에 따라 **안 줄였습니다.**
 *
 * ## 무엇이 확인됐고, 무엇이 «아직»인가
 *
 * | | |
 * |---|---|
 * | ✅ **지운 글을 맞게 잡는다** | 2026-09-17 토큰 진단: 토큰 살아 있고 미디어 144건 다 읽히는데 **그 3건만 없다.** 게다가 2026-09-12 회장님이 직접 지우신 글을 **3시간 만에** 잡아낸 기록이 우리 DB 에 있다 |
 * | ❌ **«비공개» 글을 어떻게 읽는지 모른다** | **아무도 시험하지 못했다.** 비공개 글이 「없다」로 읽히면 **멀쩡한 링크가 사라진다** |
 *
 * ⇒ **비공개 시험이 끝나면 그때 줄인다.** 권반장이 대표님께 「글 하나를 비공개로 바꿔 주십시오」를
 *   올리기로 하셨다(2026-09-17). **그 답이 이 숫자의 열쇠다.**
 *
 * ⚠ **호출이 7배가 되지 않는다 — 세어 봤다.** 한 번에 물어보는 수는 아래 `RECHECK_LIMIT`(50건)가
 *   이미 막는다. 크론은 **하루 한 번** 도니 며칠짜리로 바꾸든 **하루 최대 50건**으로 같다.
 *   ⇒ 바뀌는 것은 **「같은 글을 얼마나 자주 다시 보나」**이지 하루 총 호출 수가 아니다.
 *   🔴 **그러니 「호출이 안 는다」는 줄여도 된다는 근거가 못 된다.** 걸린 것은 요금이 아니라 **정확도**다.
 *
 * ★ 급한 자리는 크론이 아니라 **사장님이 편집화면을 열 때**가 맡는다([48]① ·
 *   `app/api/site/videos/recheck` — 감추지 않고 **알리기만**). 크론은 «그 뒤를 받치는» 그물이다.
 */
