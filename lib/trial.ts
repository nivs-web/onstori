 ...
 *
 * ── 구현 현황 (2026-09-06) ───────────────────────────────────
 *   ✅ 정지·유예·D+28 자동 삭제·삭제 전 예고 문자 → app/api/cron/expire
 *   ✅ 정기결제(빌링키) 발급·매달 청구·해지 → lib/toss.ts · api/billing/subscribe · cancel
 *   ⚠ 토스 가맹 심사(정기결제) 진행 중이라 **실호출로 검증한 적이 없다.**
 *     TOSS_SECRET_KEY 가 붙는 즉시 테스트 키로 발급·청구·해지 세 경로를 태울 것.
 *     그 전에는 checkout 이 ready:false 를 주어 결제창이 열리지 않는다.
 *   ⚠ 마이그레이션 20260906120000(billing·payments)이 아직 적용 전이다.
 */
