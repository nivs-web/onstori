...
/**
 * ★★ **「주 1회 질문을 보내 드립니다」 안내 — 문구의 단일 출처.** (2026-09-13 지시 B1)
 *
 * ⚠ 전에는 이 문장이 `app/new/wizard.tsx` 안에만 **직접 타이핑**돼 있었다. 그런데 이제
 *   **넘겨받기 화면**(`app/claim/[slug]`)에서도 같은 말을 해야 한다 — 그 화면에서도
 *   주 1회 문자가 **켜지기** 때문이다. 두 군데에 따로 적으면 한쪽만 고쳐져 화면이 거짓말을 한다
 *   (규칙 9 · 2026-09-06 에 실제로 182곳 중 175곳이 어긋났다).
 *
 * ★ 세 토막으로 나눈 이유는 가운데를 **굵게** 보여 주기 위해서다 — 뜻은 한 문장이다.
 *
 * ★★ **2026-09-15 대표님 개정 — 이 줄은 «이메일 칸» 아래로 옮겼다.**
 *   전에는 전화번호 칸 아래에 있었다. 그런데 이제 **주 1회 질문은 메일로 간다.**
 *   전화번호 칸 아래에 메일 이야기가 있으면 「이 번호로 문자가 오나?」로 읽힌다.
 *   대표님 말씀: 「전화번호 칸은 **심플하게.** 부가적으로 설명하지 말고, 기본은 공개 안 함.」
 */
export const WEEKLY_NOTICE = {
  lead: "이메일 주소로 매주 한 번 ",
  strong: "「이번 주 질문」",
  tail: "을 보내 드려요. 링크를 누르고 60초만 말씀하시면 됩니다.",
} as const;

/**
 * ★★ **「문자로도 받을 수 있다」를 알리는 한 줄.** (2026-09-15 대표님 지시)
 *
 * ★ 왜 넣나 — 대표님 말씀 그대로: 「**문자로도 받을 수 있구나** 알 수 있게 알려 주는 용도야.」
 *   기본은 메일이지만, 메일을 잘 안 보시는 사장님이 **길이 있다는 것조차 모르면** 그냥 안 하신다.
 * ★ **끄는 길을 같은 줄에 담았다.** 알리기만 하고 끌 길을 안 말하면 그것이 광고다.
 *   (옛 문구의 「언제든 끄실 수 있어요」가 여기로 들어왔다 — 문장 수를 늘리지 않으려고 합쳤다)
 */
export const WEEKLY_CHANNEL_HINT =
  "관리자 페이지에서 메일 대신 문자로 받도록 바꾸거나, 언제든 끄실 수 있습니다.";
