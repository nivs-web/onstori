...
const SHOTS = [
  { key: "pc", width: 1440, maxRatio: 6, out: 900, cropTop: 0.40 },
  { key: "phone", width: 390, maxRatio: 6, out: 360, cropTop: 0.40 },
] as const;
 ...
 * 히어로는 화면 높이 전체(100svh)이고 제목·버튼은 그 아래쪽에 있다.
 * 40% 지점부터 시작하면 제목과 버튼이 카드 안에 들어온다.
 * ⚠ 더 많이 자른다고 "웹사이트"로 보이지는 않는다 — 그건 **주소창**이 만든다(.tcard-url).
 *   2026-09-07 에 78% 까지 잘라 봤지만 여전히 사진이었다. 크롭은 시작 지점만 정한다.
 */
