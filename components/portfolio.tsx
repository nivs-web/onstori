    ...
    <section id="portfolio">
      ...
      <p className="mt-2 t-body" style={{ color: "var(--muted)" }}>
        {/* ⚠ 전에는 "안을 직접 스크롤해보세요" 였다. iframe 을 걷어낸 뒤로는 안에서 스크롤할 수
            없으니 그대로 두면 거짓말이 된다. **화면에 보이는 그대로** 적는다.
            ...
            🔴 2026-09-16 — 「마우스를 올리면 화면이 아래로 내려가요」를 **고쳤다.** 미리보기가
              마우스와 상관없이 저절로 왕복하게 바뀌어(globals.css `.tcard-play`) 그 말이
              사실이 아니게 됐다. 화면 동작을 바꾸면 이 문구도 반드시 같이 본다. */}
        <span className="say-touch">눌러서 실제 사이트를 볼 수 있어요.</span>
        <span className="say-hover">화면이 저절로 아래로 내려갔다 다시 올라와요. 누르면 진짜 사이트가 열립니다.</span>
