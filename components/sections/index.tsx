          ...
          {feed.length > 1 && (
            <p className="t-small shorts-say" style={{ margin: "0 0 var(--s-5)", color: on, opacity: 0.68 }}>
              {/* 🔴🔴 **이 문장이 [42] §3 으로 «거짓말»이 됐다 — 그래서 고친다.** (2026-09-17)
                  ...
                  🔴🔴 **손님 눈에 닿는 곳에 「몰입모드」라는 낱말을 쓰지 않는다**(2026-09-17 권반장이 정함).
                    그건 **우리끼리 부르는 이름**이다 — 손님은 그게 뭔지 모른다.
                    「숏폼시네마」도 마찬가지다(마케팅 이름이지 화면 안 문구가 아니다).
                    ⇒ 화면에는 **무엇이 일어나는지**를 쓴다: 「소리와 함께 크게 봅니다」 */}
              {feed.length}편 · 옆으로 넘겨 보세요. 영상을 누르면 소리와 함께 크게 봅니다.
            </p>
          )}
        </div>
      </div>
