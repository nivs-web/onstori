          ...
          {staged.map((it, i) => {
            ...
            <figure key={it.id} className={`stage-item${i === active ? " on" : ""}`}>
              <StageVideo
                ...
                controls={calm}
                /**
                 * 🔴🔴 **누르면 «어느 기기에서든» 열린다.** (2026-09-17 지시 [42] · 대표님 확정)
                 *
                 * > 대표님: 「**전부 보기가, 즉 몰입모드야, 몰입모드는 스피커 소리를 틀기 위해서라도
                 * >   «무조건 열리게 만들어야 함».**」
                 * > 「**어떤 상황에서든 클릭을 1번이라도 하게 되면, 소리 들으려고 클릭하거나 하면
                 * >   몰입모드로 들어감**」
                 *
                 * ⚠⚠ **[33] 의 「폰은 이대로 좋아」(= 폰 탭은 소리만)를 «이 기획이 덮어쓴다».**
                 *   그 말씀은 이번 기획 «전»의 것이다. 되돌리지 마라 — 되돌리면
                 *   **폰 손님은 12편(지금 10편) 너머를 영영 못 본다.**
                 *
                 * ⚠ **소리 끄는 길이 사라지지 않는지 먼저 쟀다**(권반장 지시):
                 *   폰에서도 스피커 단추(`.stage-sound`)가 **56×56 으로 보인다**. 소리는 그 단추가 맡는다.
                 *   (`display:none` 규칙은 「움직임 줄이기」 안에만 있다 — 거기서는 영상에 진짜 재생바가 붙는다)
                 *
                 * ⚠⚠ **`i` 가 아니라 `active` 다** — 2026-09-17 실측으로 잡았다.
                 *   편들이 **겹쳐 쌓여** 있어서 클릭은 «DOM 의 마지막 편»이 받는다.
                 *   `i` 를 쓰면 **어느 편을 눌러도 항상 마지막 편**이 열렸다(실측 7/7).
                 *   손님이 «보고 있던» 편은 언제나 `active` 다.
                 */
                onPick={() => {
                  if (calm) return;       // 「움직임 줄이기」를 켠 손님에게는 큰 화면을 안 연다
