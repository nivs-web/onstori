...
- **2026-09-05 전환 — "이야기 엔진"(기획1 `/mainplan`).** 첫 페이지·메뉴 5개(작동방식·사업이야기·자주묻는질문·리뷰·블로그)·비교 페이지를 새로 제작(`components/site/*`, 확정 히어로 "홈페이지는 텅 빈 상가입니다. 스토리에는 진짜 사람이 있습니다."). 레이아웃·섹션 순서·정보 구조는 레멘토(remento.co)를 적극 참고했고, 색상은 독자 팔레트(#005B2A/#273D3D), 아이콘·이미지·세부 UI는 유사하되 변형 — 규칙 9. **09-05 1차본의 색은 규칙 9 기준으로 조정 완료(레멘토 톤 포레스트 #1E332D·크림 #F4F0E6·라임 #E1EB6E 폐기 → 독자 팔레트 #005B2A/#273D3D, 커밋 `99f1860`) — 세부 UI(아이콘 스타일·이미지 처리·버튼 형태·카드 모서리 등)만 조정이 남아 있다(기획1 #remento 의 조정 목록).** 온보딩 5단계(`app/new/wizard.tsx`), 14일 무료→정회원 49,000원(`lib/trial.ts`, `api/billing/*`, `api/cron/expire`, `admin/members`), 60초 녹화 `/rec/{slug}?k=`(`lib/story-link.ts`, `api/story/*`). 기획실 3개: 기획1 `/mainplan` · 기획2 `/plandept` · 기획3 `/onstoriplandept`. **적용 완료(2026-09-06): 마이그레이션 `20260905090000`(신규 컬럼 7개 프로덕션 조회 확인) · Vercel env `CRON_SECRET`(만료 크론이 열려 만료 사이트가 공개에서 실제로 차단됨 — DECISIONS 2026-09-06)·`NAVER_CLIENT_ID`/`NAVER_CLIENT_SECRET`(NAVER API HUB, 온보딩 플레이스 불러오기 동작). 적용 대기로 남은 것: Vercel env `TOSS_*`(결제 모달 "준비 중") · 자막 워커(STEP 4)·발행 연동(STEP 6~7)은 `/mainplan` #plan 순서대로.**
...
9. **요금·기간 정책 (2026-09-06 회장님 최종 확정).** 값의 **단일 출처는 `lib/trial.ts`** 다 — 화면·문서 어디서도 숫자를 복사해 적지 않고 거기서 읽어 온다.
   - 요금은 **월 49,000원 구독**(매달 자동 결제). 기간의 정함이 없는 1회 결제가 **아니다.**
...
11. **디자인 시스템 (2026-09-06 회장님 개정 — 옛 "레멘토 참고 방침" 을 대체한다).**
    - **색·글자·간격·그림자·모서리·모션 시간/곡선은 `app/globals.css` 의 `:root` 토큰만 쓴다.** 정의는 `docs/DESIGN.md`·`docs/MOTION.md`.
    - **하드코딩 금지: 색상값·letter-spacing·box-shadow·border-radius·transition·animation.** 토큰에 없는 값이 필요하면 토큰을 먼저 늘린다.
    - **`docs/MOTION.md` 허용 8가지 외 애니메이션 금지.** (패럴랙스·스크롤 잭킹·글자 타이핑·숫자 카운트업·자동 슬라이드·무한 루프·커서 추적·3D 기울기 카드·로딩 스플래시)
    - **포레스트·크림·라임 조합 금지 유지** — 레멘토의 #1E332D·#F4F0E6·#E1EB6E 는 쓰지 않는다.
    - 레이아웃 배치·섹션 순서·정보 구조를 레멘토에서 참고하는 것까지는 그대로 유효하다. 문구·사진·콘텐츠는 전부 온스토리 것.
    - ⚠ 고객 사이트(`app/[slug]`)는 사장님이 고른 팔레트를 `--s-*` 로 인라인 주입한다. 그 스코프는 이 토큰과 별개다.
