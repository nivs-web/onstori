...
- **2026-09-05 전환 — "이야기 엔진"(기획1 `/mainplan`).** 첫 페이지·메뉴 5개(작동방식·사업이야기·자주묻는질문·리뷰·블로그)·비교 페이지를 새로 제작(`components/site/*`, 확정 히어로 "홈페이지는 텅 빈 상가입니다. 스토리에는 진짜 사람이 있습니다."). 레이아웃·섹션 순서·정보 구조는 레멘토(remento.co)를 적극 참고했고, 색상은 독자 팔레트(#005B2A/#273D3D), 아이콘·이미지·세부 UI는 유사하되 변형 — 규칙 9. **09-05 1차본은 색·세부 UI가 아직 레멘토 톤(포레스트/크림/라임)이라 규칙 9 기준으로 조정이 남아 있다(기획1 #remento 의 조정 목록).** 온보딩 5단계(`app/new/wizard.tsx`), 14일 무료→정회원 49,000원(`lib/trial.ts`, `api/billing/*`, `api/cron/expire`, `admin/members`), 60초 녹화 `/rec/{slug}?k=`(`lib/story-link.ts`, `api/story/*`). 기획실 3개: 기획1 `/mainplan` · 기획2 `/plandept` · 기획3 `/onstoriplandept`. **적용 완료(2026-09-06): 마이그레이션 `20260905090000`(신규 컬럼 7개 프로덕션 조회 확인) · Vercel env `CRON_SECRET`(만료 크론이 열려 만료 사이트가 공개에서 실제로 차단됨 — DECISIONS 2026-09-06)·`NAVER_CLIENT_ID`/`NAVER_CLIENT_SECRET`(NAVER API HUB, 온보딩 플레이스 불러오기 동작). 적용 대기로 남은 것: Vercel env `TOSS_*`(결제 모달 "준비 중") · 자막 워커(STEP 4)·발행 연동(STEP 6~7)은 `/mainplan` #plan 순서대로.**
