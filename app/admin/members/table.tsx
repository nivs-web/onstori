    ...
    return (
      <th key={label} className="px-3 py-2.5 font-semibold">
        <button type="button" onClick={() => setSort((s) => ({ key, desc: s.key === key ? !s.desc : true }))}
          className={`hover:underline ${on ? "text-green-700" : ""}`}>
          {label}{on ? (sort.desc ? " ▾" : " ▴") : ""}
        </button>
      </th>
    );
    ...
    <>
      <div className="mt-6 flex flex-wrap items-center gap-2">
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="상호·주소·전화·이메일 검색"
          className="ml-auto w-56 rounded-full border border-n-300 px-4 py-1.5 t-small" />
        <button type="button" onClick={csv} className="rounded-full border border-n-300 px-3.5 py-1.5 t-small font-semibold">CSV 내려받기</button>
      </div>
...
function RowView({ r, open, busy, onToggle, onPatch, onAct }: {
  ...
  return (
    <>
      ...
      {open && (
        <tr className="border-t border-n-100 bg-n-50">
