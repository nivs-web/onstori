...
export function DeleteSite({ slug, businessName }: { slug: string; businessName: string }) {
  ...
  async function call(dryRun: boolean) {
    ...
    try {
      ...
      const d = (await r.json().catch(() => ({}))) as { counts?: Record<string, number>; error?: string; ok?: boolean; filesPurged?: number };
      if (!r.ok) { setMsg(d.error ?? `실패했어요 (${r.status})`); return; }
      if (dryRun) { setCounts(d.counts ?? {}); return; }
      setDone(true);
    ...
    <div className="rounded-xl border border-danger p-3 text-left" style={{ minWidth: 280 }}>
      ...
      {counts ? (
        <p className="mt-1 t-caption leading-relaxed text-[var(--text-soft)]">
          함께 사라지는 것 — 문의 {counts.inquiries ?? 0} · 이야기 {counts.story_entries ?? 0} ·
          기록 {counts.events ?? 0} · 발행본 {counts.site_versions ?? 0} ·
          SNS 연결 {counts.sns_connections ?? 0} · 올린 기록 {counts.sns_posts ?? 0}
        </p>
      ) : (
