 ...
 *
 * ★★ **이 금지는 «사장님용 창구» 이야기다.** (2026-09-12 회장님 확정 — 구분을 여기 박아 둔다)
 *   · **사장님용**(이 파일·편집화면·손님 화면) — 삭제 창구를 **두지 않는다.** 실수 한 번이 홈페이지를 지운다.
 *   · **운영자용**(`app/api/admin/site-delete`) — **있다.** 회장님이 시험 사이트를 치우려면 길이 있어야 한다.
 *     대신 문턱을 셋 둔다: ①운영자 인증 ②**상호를 손으로 정확히 타이핑** ③**지운 기록을 표에 남긴다.**
 *   둘을 헷갈리지 마라. 「삭제 API 는 없다」가 아니라 **「사장님에게는 없다」**가 규칙이다.
