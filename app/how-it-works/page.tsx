...
export default async function HowItWorks() {
  ...
  const steps = [
    /* 🔴 **이 줄만 큰따옴표였다 — `${COPY.trialShort}` 가 손님 화면에 «글자 그대로» 나갔다.**
       (2026-09-16 권반장 지시 [12]) 옆 줄들은 전부 백틱이라 정상이었다. 백틱으로 바꿔 치환을 살린다.
     ⚠ 문장은 한 글자도 안 바꿨다. 「${COPY.trialShort}.」 자리에 `lib/trial.ts` 의 값이 들어갈 뿐이다.
     ⚠ 다른 줄에 따옴표를 쓸 때는 «치환할 것이 없는지» 먼저 본다. 이 사고가 여기서 났다. */
  ...
  return (
    <main className="min-h-svh surface-0">
      ...
      <PageHero
        ...
        sub={`글쓰기·편집·앱 설치 없이, 문자 링크 하나로 매주 60초. 그 60초가 홈페이지와 영상이 되어 ${LIVE_COUNT}곳에 퍼집니다. (나머지 ${CHANNEL_COUNT - LIVE_COUNT}곳은 준비 중)`}
      >
