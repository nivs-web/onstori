...
export function WidgetsPanel({ doc, setDoc }: { doc: SiteDocT; setDoc: (d: SiteDocT) => void }) {
  ...
  return (
    <section data-tour="panel-widgets" className="rounded-2xl border border-n-200 p-4">
      ...
      {!hasQuoteForm && (
        <p className="mt-3 rounded-xl bg-accent-soft p-3 t-caption leading-relaxed text-accent-ink">
      ...
      <div className="mt-3 space-y-3">
        ...
        <div className="rounded-xl border border-n-200 p-3">
          <div className="flex items-center justify-between gap-3">
            <span className="t-small font-semibold">전화</span>
        ...
        <div className="rounded-xl border border-n-200 p-3">
          <div className="flex items-center justify-between gap-3">
            <span className="t-small font-semibold">카카오톡</span>
          ...
          {hasQuoteForm && (
            <>
              ...
              {isOn("kakao") && !kakaoUrl && (
                <p className="mt-1 t-caption text-accent-ink">주소를 넣어야 버튼이 화면에 보여요.</p>
              )}
              {isOn("kakao") && (
                <label className="mt-2 block">
                  <span className="mb-1 block t-caption text-[var(--text-soft)]">버튼 이름 (최대 8자)</span>
