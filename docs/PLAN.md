...
- [~] **P2 진행 중** — 완료: Gemini 파이프라인(분류·카피·조립), /new 무질문 위저드, 랜딩 v0, 슬러그 검사 API, image_bank 테이블·bank 버킷, 프로덕션 생성 E2E(barun-electric.onstori.com) / 남음: 저확신 되묻기 UI(**사장님 결정 대기** — 범위 밖 업종 처리 방침). 이미지뱅크 실생성은 2026-09-01 완료(Vertex 이관 후 히어로 100장 생성·39장 승인). 완료 추가: 어드민(뱅크·사이트·쇼케이스·서브도메인 자리)·랜딩 라이브 포트폴리오(폰 프레임 iframe+탭+어드민 관리)
