...
- [~] **P2 진행 중** — 완료: Gemini 파이프라인(분류·카피·조립), /new 무질문 위저드, 랜딩 v0, 슬러그 검사 API, image_bank 테이블·bank 버킷, 프로덕션 생성 E2E(barun-electric.onstori.com) / 남음: 이미지뱅크 벤치마크·구축(당근 첫 반응 후), 어드민 최소판, 쇼케이스 전시, 저확신 되묻기 UI
- [~] **P3 진행 중** — 완료(슬라이스1): /{slug}/edit 폼 에디터·스토리 작성(사진 업로드)·draft/발행 분리·완성도 점수 실계산·anonId+운영자 소유권 / 남음: 사진 갤러리·메뉴판 편집, 섹션 추가/순서, 앵커 스크롤 연결, 투어 최소 동작
