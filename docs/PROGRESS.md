...
4. 히어로 한정 재대결 → 승자 모델로 500장 웨이브 → `/admin/bank` 검수
...
**잠정 결론: 기본은 Flash.** 동일 프롬프트 대결에서 Flash가 피사체(장갑 낀 손·렌치·비계 클램프)를 분명히 잡아 "작업 중"이 즉시 읽힌 반면, Pro는 먼지 덮개가 화면을 지배해 주제가 약했다. 표본이 모델당 6장이라 확정은 아님. 크레딧이 있어 비용은 결정 변수가 아니므로 **품질로 판단해야 하고, 현 표본으로 3.4배를 정당화할 근거 없음.** DECISIONS 2026-08-31의 "pro=히어로 / flash=갤러리" 병행안은 히어로 한정 재대결(각 10장) 후 확정.
비교 리포트(이미지 포함): https://claude.ai/code/artifact/3cbb3ccb-fe41-4ce2-8fc3-7b48e1313c09

부수 수정: `bench-image.ts`가 mjs→ts 이관 시 top-level await로 CJS에서 깨지던 것 → `main()` 래핑.
