...
## 다음 세션 시작점 (2026-09-01 밤 기준)

P0·P1·P3·**P4 완료**. P2만 항목 하나 남았고 그건 코드가 아니라 **사장님 결정 대기**다.

**다음 Phase는 P5(결제)** 인데, 착수 조건이 코드가 아니라 서류다 — 아래 ①이 풀리기 전엔 P5를 시작할 수 없다.

| 순위 | 할 일 | 성격 | 막고 있는 것 |
|---|---|---|---|
| ① | **통신판매업 신고 → 토스페이먼츠 가맹** | 사장님 | **P5 전체.** 1개월 무료 종료 시점에 첫 결제가 발생하므로 지금 시작해야 타이밍이 맞는다 |
| ② | **v1 범위 밖 업종 처리 방침 결정** (되묻기 / 차단 / 범용 수용) | 사장님 | P2 잔여 + **당근 홍보**. 불특정 업종이 들어오면 바로 드러난다 (아래 TODO 참조) |
| ③ | **남은 이미지 웨이브 — 비히어로**(갤러리·시공사례·about·process, Flash 모델) | 운영 | 없음. **크레딧 확인으로 막혀 있던 게 2026-09-01 풀렸다.** 뱅크 131장 중 124장이 hero라 비히어로가 사실상 비어 있고, 그동안은 Unsplash 플레이스홀더로 폴백한다. 크레딧 만료 **2026-12-01** — 안 쓰면 사라진다. 생성 후 `/admin/bank`에서 검수·승인해야 실제로 쓰인다(생성분은 전부 검수 대기로 들어간다) |
| ④ | **`/api/generate` rate limit** | 코드 | **공개 홍보.** LLM 호출 API가 지금 무방비다 (아래 "그 외") |
| ⑤ | **WIF 전환** (2~3시간) | 코드 | 없음. P5 진입 전 검토 항목 — 유일하게 남은 장기 키를 없앤다 |
| ⑥ | **P3 이월 3건**: 섹션 삭제 · 앵커 스크롤 · 투어 스텝 재정의 | 코드 | 없음. 아무것도 막고 있지 않은 제품 완성도 항목 — 아래 "P3에서 이월한 항목" 표에 건드릴 파일까지 적혀 있다 |
| ⑦ | 운영자 인증 교체 + 운영자 로그아웃 라우트 | 코드 | 없음. **P7 묶음**으로 이월 결정 (2026-09-01) |

**막힘 없이 지금 손댈 수 있는 건 ③·④·⑤·⑥이다.** ①②는 사장님 판단이 있어야 움직이고, ⑦은 P7로 미루기로 이미 정했다. 이 중 **③만 기한이 있다**(크레딧 만료 2026-12-01).

~~크레딧 확인~~ — ✅ **2026-09-01 확인 완료. 크레딧이 정상 상쇄된다**(₩435,523 중 ₩25,243.93 사용, 잔액 ₩410,279). 남은 이미지 웨이브를 막던 조건이 풀렸다.
...
**뱅크 재고 (2026-09-02 06:06 실측, 비히어로 웨이브 직전)**: 전체 133장 — **hero 124장 전량 승인 · gallery 9장(승인 7) · about 0 · process 0**. 비히어로가 사실상 비어 있어서 신규 사이트의 갤러리·about·process는 Unsplash 플레이스홀더로 폴백한다. 히어로 100장 웨이브 결과는 아래 "AI 스택" 절 참조.
