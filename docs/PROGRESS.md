...
4. ~~로그아웃 UI~~ — ✅ 완료 (위)
5. **403 거부화면 문구 루프** (배너 작업 검증에서 발견, 범위 밖으로 남김): 기기1에서 익명으로 만든 미claim 사이트를 기기2에서 **로그인한 채로** 열면 `owner_id` null + anon_id 불일치로 403 → "이 홈페이지 주인이라면 로그인 후 수정할 수 있어요"가 뜬다. 이미 로그인한 사람에게 로그인하라는 문구이고, 눌러도 `/login`이 세션을 발견해 곧장 되돌려보내 **같은 화면으로 도는 루프**가 된다. 최소 수정: `/api/site/get`이 403일 때 body에 `signedIn`만 실어 보내고, 로그인 상태면 "이 계정에는 연결돼 있지 않아요 — 처음 만든 기기에서 [내 계정에 연결하기]" + `/my` 링크로 분기 (`app/[slug]/edit/ui.tsx` 거부화면)
6. **운영자 로그아웃 라우트 부재**: `onstori_admin`은 httpOnly + `secure`로 심기는데(`app/api/admin/login/route.ts`) 지우는 경로가 어디에도 없다. `/admin`에도 로그아웃 UI가 없어 한 번 인증하면 30일간 그 브라우저의 모든 소유 판정이 admin으로 고정된다 — 검증할 때마다 사람을 헷갈리게 하고(2026-09-01 실제로 겪음), 공용 PC에서는 권한이 남는 문제이기도 하다. 최소 수정: `POST /api/admin/logout`이 `res.cookies.delete("onstori_admin")` 하고 `/admin`에 버튼 하나. 운영자 인증을 Supabase Auth로 교체(3번)할 때 함께 처리해도 된다.
